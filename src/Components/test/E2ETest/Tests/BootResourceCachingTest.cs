    ...
    : ServerTestBase<AspNetSiteServerFixture>
{
    ...
    public void CachesResourcesAfterFirstLoad()
    {
        ...
        Assert.NotEmpty(initialResourcesRequested.Where(path => path.EndsWith("System.Private.CoreLib.wasm", StringComparison.Ordinal)));
        Assert.Empty(initialResourcesRequested.Where(path => path.EndsWith(".dll", StringComparison.Ordinal)));
        ...
        Assert.Empty(subsequentResourcesRequested.Where(path => path.EndsWith(".wasm", StringComparison.Ordinal)));
    }
